...
public class QualifiedAccountNameCursorAdapter extends SimpleCursorAdapter {
     ...
     * @param spinnerView
     */
    public static void hideFavoriteAccountStarIcon(@Nullable View spinnerView) {
        if (spinnerView == null) return;
        TextView textViewWithStarIcon = spinnerView.findViewById(R.id.text2);

        if (textViewWithStarIcon == null) return;
     ...
     * @param spinnerSelectedItemLayout Layout resource for selected item text
     */
    public QualifiedAccountNameCursorAdapter(Context context,
        ...
        super(context,
                ...
                cursor,
                new String[]{DatabaseSchema.AccountEntry.COLUMN_FULL_NAME, DatabaseSchema.AccountEntry.COLUMN_NAME},
        ...
        setFilterQueryProvider(constraint -> {
            ...
            whereArgsAsList.add("%" + ((constraint != null) ? constraint.toString() : "") + "%");

            // Convert List into WhereArgs
            final String[] whereArgs = whereArgsAsList.toArray(new String[0]);
            ...
            final String where = (getCursorWhere() != null ? getCursorWhere() + " AND " : "") +
                    DatabaseSchema.AccountEntry.COLUMN_FULL_NAME + " LIKE ?";

            return accountsDbAdapter.fetchAccountsOrderedByFavoriteAndFullName(where, whereArgs);
        });
    }
     ...
     * @param context                   Application context
     * @param cursor                    Cursor to account data
     * @param selectedSpinnerItemLayout Layout resource for selected item text
     */
    public QualifiedAccountNameCursorAdapter(Context context,
                                             Cursor cursor,
                                             String cursorWhere,
        ...
        // Layout of the closed spinner item
        this(context, cursor, cursorWhere, cursorWhereArgs, selectedSpinnerItemLayout, R.layout.account_spinner_dropdown_item_2lines);
    }
     ...
     */
    public QualifiedAccountNameCursorAdapter(Context context, Cursor cursor, String cursorWhere, String[] cursorWhereArgs) {
        // Layout of the closed spinner item
        this(context, cursor, cursorWhere, cursorWhereArgs, android.R.layout.simple_spinner_item);
    }
    ...
    public View getView(final int position, final View convertView, final ViewGroup parent) {
        ...
        if (parent.getId() != R.id.toolbar_spinner) {
            ...
            setTextColorAccordingToAccountUID(view, accountUID);
        } else {
        ...
        displayFavoriteAccountStarIcon(view, isFavorite);
     ...
     * <p>
     * See https://stackoverflow.com/questions/17458251/android-attempt-to-re-open-an-already-closed-object-sqlitequery-using-loaderm
     *
     * @param cursor
     */
    @Override
     ...
     *
     * @param accountFullName Account full name
    ...
    String getParentAccountFullName(final String accountFullName) {
        ...
        if (index > 0) {
            ...
            parentAccountFullName = accountFullName.substring(0, index);
        } else {
     ...
     *
     * @param view       View containing text field to colorize
     * @param accountUID Account UID
     */
    void setTextColorAccordingToAccountUID(final View view, final String accountUID) {
        // Set color on text1 (selected spinner item)
        final TextView simpleAccountNameTextView = view.findViewById(android.R.id.text1);
        AccountUtils.setAccountTextColor(simpleAccountNameTextView, accountUID);

        // Set color on text2
        final TextView simpleAccountNameTextView2 = view.findViewById(R.id.text2);
        AccountUtils.setAccountTextColor(simpleAccountNameTextView2, accountUID);
    }
     ...
     */
    void displayFavoriteAccountStarIcon(View spinnerSelectedItemView, Integer isFavoriteAccount) {
        TextView simpleAccountNameTextView = spinnerSelectedItemView.findViewById(R.id.text2);

        // Do nothing if the field is missing
        if (simpleAccountNameTextView == null) return;
        ...
        if (isFavoriteAccount == 0) {
            ...
            simpleAccountNameTextView.setCompoundDrawablesWithIntrinsicBounds(0, 0, R.drawable.ic_star_black_18dp, 0);
        }
    }
